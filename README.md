...
- Express verifies the token and does DB operations on **Supabase PostgreSQL**
...
**`server/src/middleware/authMiddleware.js`** — Every request to Express goes through this. It reads the JWT token, verifies it with Supabase, and only then allows access.
